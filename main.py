...
def mainfunction():
    ...
    n1=Label(add_frame,text="date J-M-A")
    ...
    def adding() :
          ...
          b = ldate[i] + timedelta(days=7)

          y=int(a3.get())
          tre.append(y)
          r=0
          if i>0:
              nbj=int((datetime(ldate[i].year,ldate[i].month,ldate[i].day)-datetime(ldate[i-1].year,ldate[i-1].month,ldate[i-1].day)).days)
              r=nbj*tre[i-1]*lsolde[i-1]/36000

          operation.append(a2.get())
          solde+=int(operation[i])
          lsolde.append(solde)



          my_tree.insert(parent='',index='end',text="",values=(ldate[i],b,operation[i],0,solde,nbj,y,r))
